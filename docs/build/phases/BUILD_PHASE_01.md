# BUILD PHASE 01: Resolve dependency provenance and pin decisions

## Phase Identity

- Track: Build System Migration; phase 01; status: NOT STARTED.
- Kind: **DEPENDENCY PREPARATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-00-approved`.
- Technical prerequisites: 00. These do not permit skipping the serial predecessor.

## Goal

Choose reproducible dependency/tool versions and ownership from measured reference evidence.

## Why This Phase Exists

Assimp ABI is unknown, GLM is a local 0.9.9.9 tree, SDL_ttf header/DLL versions disagree, and host Conan may not represent installed MSVC.

## Preconditions

Phase 00 is fully approved/published; HEAD equals build-phase-00-approved peeled commit; explicit START BUILD PHASE 01.
...
docs/build/dependencies/ provenance and pin-decision documents; audit A6/A8 corrections supported by evidence; no recipe implementation.
...
No package install into production, Conan recipe/profile, CMake target, SDK redistribution, or silent version upgrade. Unavailable matching recipes require separately reviewed preparation.

## Expected Changes

Record exact proposed Conan client/CMake/compiler/SDK pins, approved remotes, package/version/recipe availability, fmt mode, SDL/Assimp/TBB build options, FMOD SDK source and hashes; identify transitive FreeType/zlib and unused vendors.
Produce one understandable diff and `../reviews/BUILD_PHASE_01_REVIEW.md` with
...
Check primary package recipes/releases against local headers and binary imports; confirm actual MSVC-to-Conan settings compatibility. Compare header/DLL/import-lib identities, licenses and architecture. Verify all active dependencies have one owner and every unknown is a named blocking decision. Validate document links/data without rerunning application workloads.
...
Use Phase 00 import/compile evidence; no source/build behavior change. Proposed dependency differences must have an explicit compatibility validation plan.
...
APPROVE BUILD PHASE 01 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-01-approved`
...
Verify `build-phase-00-approved` and its recorded peeled commit. Restore only the exact owned
...
Any unresolved version/ABI/provenance needed by the next dependency phase, unavailable recipe, unapproved SDK origin or incompatible tool pin.
