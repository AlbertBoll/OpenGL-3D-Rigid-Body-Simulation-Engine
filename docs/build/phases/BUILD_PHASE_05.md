# BUILD PHASE 05: Isolate the vendored implementation targets

## Phase Identity

- Track: Build System Migration; phase 05; status: NOT STARTED.
- Kind: **VENDOR / PCH BOUNDARY PREPARATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-04-approved`.
- Technical prerequisites: 02, 03, 04. These do not permit skipping the serial predecessor.

## Goal

Give GLAD, ImGui/backends, ImGuizmo and stb explicit compiled target ownership independent of engine PCH.

## Why This Phase Exists

Twelve vendor TUs are currently embedded in GEngine.lib and most include gepch.h.

## Preconditions

Phase 04 is fully approved/published; HEAD equals build-phase-04-approved peeled commit; explicit START BUILD PHASE 05.
...
cmake/vendor/ target definitions; exact active vendor TU prerequisite/PCH includes; premake5.lua per-file PCH exceptions if needed; vendor ownership records.
...
No vendor upgrade/regeneration, source algorithm edits or unused backend explosion. Do not link vendor libraries to Core just to recover gepch.h. Keep license notices.

## Expected Changes

Create STATIC C GLAD and meaningful ImGui-core/ImGuizmo/stb C++ targets with explicit prerequisites. Preserve active ImGuizmo and reviewed GLM instantiations. Record backend source ownership now; attach SDL/OpenGL backend targets in Phase 06 after their package prerequisites exist.
Produce one understandable diff and `../reviews/BUILD_PHASE_05_REVIEW.md` with
...
Build introduced vendor targets Debug/Release without engine PCH and verify unique implementation symbols plus correct C/C++ and CRT commands. Validate backend prerequisite cleanup through Premake; CMake backend attachment belongs to Phase 06. Build affected Premake targets in both configs after PCH exceptions and classify vendor warnings separately. Verify intended SDL/ImGui header selection.
...
Compare exact compiled vendor source set, public macros and versions with Phase 00; preserve working loader/backends and engine warning baseline.
...
APPROVE BUILD PHASE 05 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-05-approved`
...
Verify `build-phase-04-approved` and its recorded peeled commit. Restore only the exact owned
...
Vendor requires engine runtime/logging, duplicate symbols/copies appear, SDL headers drift, or isolation requires more than explicit prerequisite/PCH cleanup.
