# BUILD PHASE 09: Model the local FMOD SDK with imported targets

## Phase Identity

- Track: Build System Migration; phase 09; status: NOT STARTED.
- Kind: **SDK INTEGRATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-08-approved`.
- Technical prerequisites: 01, 03. These do not permit skipping the serial predecessor.

## Goal

Provide verified FMOD::Core and FMOD::Studio configuration-aware imported targets.

## Why This Phase Exists

Current apps link logging and non-logging libraries together and use tracked bin DLLs as implicit runtime origin.

## Preconditions

Phase 08 is fully approved/published; HEAD equals build-phase-08-approved peeled commit; explicit START BUILD PHASE 09.
...
cmake/FindFMOD.cmake or equivalent SDK adapter; SDK validation probes/metadata; local-path usage documentation. Proprietary SDK bytes stay outside the checkpoint.
...
No download/redistribution, license bypass, newer FMOD API conversion or global PATH discovery. Test only declared SDK origin and exact reference-approved pairs.

## Expected Changes

Accept explicit FMOD_SDK_ROOT, validate 1.9.9-compatible headers/import libraries/DLLs/x64/config pairs and Studio-to-Core dependency; fail with useful missing-SDK diagnostics.
Produce one understandable diff and `../reviews/BUILD_PHASE_09_REVIEW.md` with
...
Configure valid/missing/wrong-architecture/mismatched-version SDK fixtures; link a minimal Core/Studio probe in Debug/Release, stage only its verified runtime in a disposable directory and run initialization/shutdown. Verify imported target properties against actual library DLL names.
...
Match reference FMOD header/binary identity and bank compatibility. Do not remove existing postbuild FMOD checks.
...
APPROVE BUILD PHASE 09 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-09-approved`
...
Verify `build-phase-08-approved` and its recorded peeled commit. Restore only the exact owned
...
Missing authorized SDK, mismatched import/DLL pair, unknown version/architecture, incompatible banks or inability to establish origin.
