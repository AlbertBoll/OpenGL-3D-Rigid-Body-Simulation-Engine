# BUILD PHASE 14: Extract the Physics kernel target

## Phase Identity

- Track: Build System Migration; phase 14; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-13-approved`.
- Technical prerequisites: 13. These do not permit skipping the serial predecessor.

## Goal

Create the STATIC GEngine::Physics kernel with unique translation-unit ownership.

## Why This Phase Exists

Physics consumers currently depend on a monolithic archive and ShapeConvex is compiled twice across engine/test targets.

## Preconditions

Phase 13 is fully approved/published; HEAD equals build-phase-13-approved peeled commit; explicit START BUILD PHASE 14.
...
GEngine/modules/Physics/CMakeLists.txt; exact Physics source ledger; small kernel validation consumer and build metadata. Existing Physics algorithms are read-only.
...
No Scene/Render/Audio/UI/SDL/GLAD/FMOD dependency; no duplicate implementation in probes. Existing Premake test target duplication remains until explicitly reconciled in the test migration.

## Expected Changes

Add ge_physics STATIC with Core/Math only, own ShapeConvex once in the CMake path, and expose consistent profiling options. Bounds stays owned by Math.
Produce one understandable diff and `../reviews/BUILD_PHASE_14_REVIEW.md` with
...
Build kernel and real body/world/shape consumer Debug/Release without graphics/audio SDK roots; inspect actual commands and final imports/link symbols. Exercise deterministic body/shape lifecycle and compare existing Premake tests/benchmark. Check one owner for every Physics TU and profiling on/off definitions.
...
Preserve runtime body identity/layout/step outcomes and all reference warning/language/CRT gates.
...
APPROVE BUILD PHASE 14 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-14-approved`
...
Verify `build-phase-13-approved` and its recorded peeled commit. Restore only the exact owned
...
Kernel needs upper modules or graphics SDKs, a missing symbol tempts duplicate sources/linker tricks, or test/benchmark outcomes change.
