# BUILD PHASE 16: Extract the Graphics target

## Phase Identity

- Track: Build System Migration; phase 16; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-15-approved`.
- Technical prerequisites: 15. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Graphics for GPU buffers, shaders/textures/framebuffers and font resources.

## Why This Phase Exists

These implementations are compiled but spread across Mesh, Geometry, Assets, Core and Managers folders.

## Preconditions

Phase 15 is fully approved/published; HEAD equals build-phase-15-approved peeled commit; explicit START BUILD PHASE 16.
...
GEngine/modules/Graphics/CMakeLists.txt; exact assigned sources/headers and prerequisite includes; graphics validation consumer.
...
No high-level renderer/entity/material ownership, no whole Assets/Core/Geometry directory glob, no Scene/UI/Application back-edge.

## Expected Changes

Add ge_graphics STATIC with GLAD/SDL/TTF/stb and foundational target requirements; explicit ownership of GPU Geometry base and caches from Phase 15.
Produce one understandable diff and `../reviews/BUILD_PHASE_16_REVIEW.md` with
...
Build PCH-free public-header and GL-context resource consumers Debug/Release; inspect source sets/CRT/warnings. Create/delete buffers, textures, framebuffer and representative shader/font resources from a declared test CWD. Validate lower-module closure and compare affected Premake behavior.
...
GPU resource names/types and observable shader/font/framebuffer outcomes must match reference; record any known reference runtime limitations.
...
APPROVE BUILD PHASE 16 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-16-approved`
...
Verify `build-phase-15-approved` and its recorded peeled commit. Restore only the exact owned
...
Context/resource ownership changes, high-level dependency survives, missing vendor symbols or shader/texture regression.
