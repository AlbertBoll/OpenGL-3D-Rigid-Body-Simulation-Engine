# BUILD PHASE 17: Extract procedural Geometry

## Phase Identity

- Track: Build System Migration; phase 17; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-16-approved`.
- Technical prerequisites: 16. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Geometry for procedural shapes and spatial geometry utilities.

## Why This Phase Exists

Shape generation is a meaningful compiled owner but its GPU Geometry base and neutral Bounds belong below it.

## Preconditions

Phase 16 is fully approved/published; HEAD equals build-phase-16-approved peeled commit; explicit START BUILD PHASE 17.
...
GEngine/modules/Geometry/CMakeLists.txt; ledger-assigned Shapes/Curve/Surface/Terrain/Box/KDTree files and required include repairs; geometry consumers.
...
No mesh-generation algorithm or terrain math correction. Do not introduce an Animation dependency through high-level registration.

## Expected Changes

Add ge_geometry STATIC using Graphics/Math/Core and Physics for the actual public Diamond/Convex helpers; leave ShapeManager/Factory registration of models/animation above this module.
Produce one understandable diff and `../reviews/BUILD_PHASE_17_REVIEW.md` with
...
Build Debug/Release procedural consumers and PCH-free headers. Compare box/terrain/sphere/Diamond vertex/index/bounds outputs with reference; exercise GPU-backed geometry. Verify unique Bounds/GPU Geometry ownership and the real Geometry-to-Physics edge without a Physics-to-Geometry back-edge; preserve existing convex algorithms.
...
Geometry arrays, layouts and bounds remain equivalent; use existing tests where they cover terrain/shape math.
...
APPROVE BUILD PHASE 17 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-17-approved`
...
Verify `build-phase-16-approved` and its recorded peeled commit. Restore only the exact owned
...
Registration creates a module cycle, public templates require upper-level symbols or generated geometry differs.
