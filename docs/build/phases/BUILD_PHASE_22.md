# BUILD PHASE 22: Extract the ECS Scene target

## Phase Identity

- Track: Build System Migration; phase 22; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-21-approved`.
- Technical prerequisites: 21. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Scene with the real ECS, camera-data and physics integration closure.

## Why This Phase Exists

A separately testable ECS library now has the required boundary evidence; legacy rendering scenes deliberately remain in Render.

## Preconditions

Phase 21 is fully approved/published; HEAD equals build-phase-21-approved peeled commit; explicit START BUILD PHASE 22.
...
GEngine/modules/Scene/CMakeLists.txt; ledger-assigned ECS/camera-data/component TUs and explicit include prerequisites; Scene validation consumer.
...
Do not claim graphics-free Scene if mesh/shader types still require Graphics; do not add legacy Core/Scene.cpp or controller implementations by folder glob.

## Expected Changes

Add ge_scene STATIC with Core/Math/Events/Physics and actual Geometry/Graphics dependencies, explicit public EnTT types and no Render/UI/Audio.
Produce one understandable diff and `../reviews/BUILD_PHASE_22_REVIEW.md` with
...
Build and link actual Scene creation/copy/physics lifecycle consumers Debug/Release without Render/UI/FMOD targets. Run retained focused/full regression comparisons; inspect source ownership and final module graph for cycles. Verify pure Physics consumers still omit Scene.
...
Compare ECS runtime and mesh/physics integration outcomes with current Premake and source-tag reference.
...
APPROVE BUILD PHASE 22 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-22-approved`
...
Verify `build-phase-21-approved` and its recorded peeled commit. Restore only the exact owned
...
Hidden Render/controller/virtual references, missing neutral component definitions or an attempt to solve unresolved symbols with duplicate TUs.
