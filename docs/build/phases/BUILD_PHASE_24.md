# BUILD PHASE 24: Invert the Platform and UI lifecycle boundary

## Phase Identity

- Track: Build System Migration; phase 24; status: NOT STARTED.
- Kind: **CYCLE / BOUNDARY PREPARATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-23-approved`.
- Technical prerequisites: 15, 23. These do not permit skipping the serial predecessor.

## Goal

Remove mandatory ImGui ownership from Window/SDLWindow while preserving UI/context lifecycle.

## Why This Phase Exists

Window includes ImGuiWindow and SDLWindow constructs it; ImGuiWindow consumes SDLWindow and GL backend state.

## Preconditions

Phase 23 is fully approved/published; HEAD equals build-phase-23-approved peeled commit; explicit START BUILD PHASE 24.
...
Core/Window and Windows/SDLWindow/ImGuiWindow boundaries; BaseApp/EventManager UI lifecycle hookup and exact direct callers.
...
No context-version, window property, MSAA, docking, viewport, event-capture or resource-lifetime changes; no SHARED plugin solution.

## Expected Changes

Make UI initialization/event forwarding/frame/shutdown explicit composition hooks; Platform retains SDL GL-context ownership; UI consumes Platform without a reverse link.
Produce one understandable diff and `../reviews/BUILD_PHASE_24_REVIEW.md` with
...
Build all affected Premake apps Debug/Release; verify context creation, ImGui input/docking/render/shutdown order and missing-font behavior against reference. Compile/link a Platform-only window consumer without ImGui/Render and inspect dependency graph.
...
Preserve SDL OpenGL 4.6 request and current UI startup/frame/teardown behavior, including app-specific properties.
...
APPROVE BUILD PHASE 24 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-24-approved`
...
Verify `build-phase-23-approved` and its recorded peeled commit. Restore only the exact owned
...
UI still required to link Platform, context lifetime/order differs or callback seam needs a wider behavioral redesign.
