# BUILD PHASE 25: Extract Platform window and context services

## Phase Identity

- Track: Build System Migration; phase 25; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-24-approved`.
- Technical prerequisites: 24. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Platform for SDL windows/context and WindowManager.

## Why This Phase Exists

The prepared window/UI boundary permits low-level Platform ownership without pulling application/render services.

## Preconditions

Phase 24 is fully approved/published; HEAD equals build-phase-24-approved peeled commit; explicit START BUILD PHASE 25.
...
GEngine/modules/Platform/CMakeLists.txt; ledger-listed Window/SDLWindow/WindowManager sources and prerequisite includes; window probe.
...
No Input/Application/Render/UI back-edge; no change to window/context or system SDK baseline.

## Expected Changes

Add ge_platform STATIC with SDL2/GLAD and foundational/event requirements; express SDL main policy per executable.
Produce one understandable diff and `../reviews/BUILD_PHASE_25_REVIEW.md` with
...
Build/launch minimal Platform window probe Debug/Release from declared runtime origin; inspect DLL imports and C++20/CRT/warnings. Exercise create/resize/close and context setup; verify no ImGui/FMOD/Render closure. Recheck affected Premake windows.
...
Match reference window settings/context requirements and lifecycle behavior.
...
APPROVE BUILD PHASE 25 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-25-approved`
...
Verify `build-phase-24-approved` and its recorded peeled commit. Restore only the exact owned
...
Window probe requires upper modules, wrong GL context/SDK or missing runtime origin.
