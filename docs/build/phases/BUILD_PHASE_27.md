# BUILD PHASE 27: Extract Audio with FMOD ownership

## Phase Identity

- Track: Build System Migration; phase 27; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-26-approved`.
- Technical prerequisites: 09, 10. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Audio independently of graphics and application composition.

## Why This Phase Exists

AudioSystem/SoundEvent are a coherent compiled subsystem with a proprietary SDK boundary.

## Preconditions

Phase 26 is fully approved/published; HEAD equals build-phase-26-approved peeled commit; explicit START BUILD PHASE 27.
...
GEngine/modules/Audio/CMakeLists.txt; Audio explicit prerequisites; bank/runtime validation consumer and docs.
...
No audio engine upgrade, bank conversion, new resource API or implicit requirement that every app links Audio.

## Expected Changes

Add ge_audio STATIC with precise FMOD::Studio/Core visibility; retain bank/event APIs and explicit per-config SDK locations.
Produce one understandable diff and `../reviews/BUILD_PHASE_27_REVIEW.md` with
...
Build and run bank initialization/event/update/shutdown smoke in Debug/Release from declared SDK/resource roots. Verify correct Studio/Core DLL pair, no PATH fallback and no Render/UI closure. Compare existing audio-enabled Premake apps.
...
Match reference bank compatibility, event identity and audio startup/shutdown; distinguish unavailable audio-device environment from build failure.
...
APPROVE BUILD PHASE 27 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-27-approved`
...
Verify `build-phase-26-approved` and its recorded peeled commit. Restore only the exact owned
...
Missing SDK/bank origin, wrong DLL pair/version, audio behavior drift or unwanted graphics dependency.
