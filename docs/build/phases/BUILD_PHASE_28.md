# BUILD PHASE 28: Extract UI and its backend integration

## Phase Identity

- Track: Build System Migration; phase 28; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-27-approved`.
- Technical prerequisites: 05, 16, 25. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::UI for UICore and the prepared ImGui window integration.

## Why This Phase Exists

UI consumes low-level graphics/platform but does not require the high-level Render module.

## Preconditions

Phase 27 is fully approved/published; HEAD equals build-phase-27-approved peeled commit; explicit START BUILD PHASE 28.
...
GEngine/modules/UI/CMakeLists.txt; UICore/ImGuiWindow explicit prerequisites and UI smoke consumer.
...
No ImGui upgrade, visual redesign or Render diagnostic implementation in UI. Do not restore Platform-to-UI linkage.

## Expected Changes

Add ge_ui STATIC linked to intended ImGui/ImGuizmo backend targets, Platform and Graphics; preserve active vendor copy and font inputs.
Produce one understandable diff and `../reviews/BUILD_PHASE_28_REVIEW.md` with
...
Build/run a small UI window in both configs, load declared fonts, exercise an existing widget/gizmo path and capture correct frame/shutdown order. Verify no Render/Application/FMOD dependency and unique ImGui implementations.
...
Compare reference UI font/capture/docking settings and warnings; retain known visual defects unchanged.
...
APPROVE BUILD PHASE 28 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-28-approved`
...
Verify `build-phase-27-approved` and its recorded peeled commit. Restore only the exact owned
...
UI needs Render, duplicate backend implementation, font/resource regression or lifecycle inversion is incomplete.
