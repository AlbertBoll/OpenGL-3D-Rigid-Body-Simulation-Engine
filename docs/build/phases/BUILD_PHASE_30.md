# BUILD PHASE 30: Extract the Render target

## Phase Identity

- Track: Build System Migration; phase 30; status: NOT STARTED.
- Kind: **MODULE EXTRACTION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-29-approved`.
- Technical prerequisites: 08, 29. These do not permit skipping the serial predecessor.

## Goal

Create GEngine::Render with explicit lower-module ownership and no application back-edge.

## Why This Phase Exists

The largest remaining coherent subsystem spans RenderSystem, legacy scene rendering, materials, controllers and software ray tracing.

## Preconditions

Phase 29 is fully approved/published; HEAD equals build-phase-29-approved peeled commit; explicit START BUILD PHASE 30.
...
GEngine/modules/Render/CMakeLists.txt; ledger-listed renderer/material/legacy scene/entity/light/sprite/character/camera/extras/manager/factory prerequisites; render probe metadata.
...
No engine-wide archive or PUBLIC-all dependencies. Do not change rendering algorithms, physics debug rendering semantics or target ownership to evade unresolved symbols.

## Expected Changes

Add ge_render STATIC using prepared module closures; express TBB/GLAD and exposed requirements precisely. Account for every listed TU without including BaseApp/GEngine.
Produce one understandable diff and `../reviews/BUILD_PHASE_30_REVIEW.md` with
...
Build Debug/Release target and real render consumer; verify full source ledger, link/import closure, C++20/CRT/warnings and no Application reference. Compare representative 2D, ECS, legacy scene and SimpleRenderer output/flow with Premake evidence.
...
Preserve rendering and camera/shape registration behavior, including baseline-known rendering defects.
...
APPROVE BUILD PHASE 30 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-30-approved`
...
Verify `build-phase-29-approved` and its recorded peeled commit. Restore only the exact owned
...
Unresolved symbols/cycles require duplicate TUs or library repeats, missed legacy ownership or visual/runtime regression.
