# BUILD PHASE 35: Stage resources and declare application working directories

## Phase Identity

- Track: Build System Migration; phase 35; status: NOT STARTED.
- Kind: **RESOURCE / CWD SAFEGUARD**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-34-approved`.
- Technical prerequisites: 34. These do not permit skipping the serial predecessor.

## Goal

Provide deterministic resource manifests and CWD metadata for each app without changing resource APIs.

## Why This Phase Exists

Shared managers, UI fonts and FMOD banks use paths relative to app source folders; Breakout also has local levels/images.

## Preconditions

Phase 34 is fully approved/published; HEAD equals build-phase-34-approved peeled commit; explicit START BUILD PHASE 35.
...
cmake resource staging/install definitions; per-app resource/CWD metadata; resource validation fixtures/docs.
...
No resource loader redesign, asset edits, blanket copying repository, or dependence on source CWD by accident. Protect model .obj files from cleanup.

## Expected Changes

Stage the necessary tracked hierarchy under owned run roots, preserving current relative resolutions; record shader/font/model/texture/bank/level origins and required startup files.
Produce one understandable diff and `../reviews/BUILD_PHASE_35_REVIEW.md` with
...
Validate every declared startup resource resolves from each app CWD in fresh Debug/Release staging; hash source/staged contents. Test invocation from unrelated cwd and paths containing spaces; missing-resource fixtures fail actionably. Verify clean/staging roots cannot touch tracked source/Premake bin.
...
Compare all four source-CWD resource resolutions and shared/local layouts with reference; distinguish confirmed baseline missing-resource cases from migration omissions.
...
APPROVE BUILD PHASE 35 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-35-approved`
...
Verify `build-phase-34-approved` and its recorded peeled commit. Restore only the exact owned
...
A source path cannot be reproduced without behavioral API change, resource origin/licensing is unknown or missing baseline resources require an additional preparation decision.
