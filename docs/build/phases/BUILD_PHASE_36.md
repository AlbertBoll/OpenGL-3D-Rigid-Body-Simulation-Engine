# BUILD PHASE 36: Migrate GEngineEditor

## Phase Identity

- Track: Build System Migration; phase 36; status: NOT STARTED.
- Kind: **APPLICATION MIGRATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-35-approved`.
- Technical prerequisites: 31, 35. These do not permit skipping the serial predecessor.

## Goal

Add the editor executable with explicit module, SDK and resource closure.

## Why This Phase Exists

Editor uses legacy scenes, animation/model import, panels/ImGuizmo and direct FMOD calls; it is a broad integration consumer.

## Preconditions

Phase 35 is fully approved/published; HEAD equals build-phase-35-approved peeled commit; explicit START BUILD PHASE 36.
...
GEngineEditor/CMakeLists.txt; editor runtime/resource metadata; exact build-prerequisite includes if required. Editor feature implementation is otherwise read-only.
...
No editor feature/renderer/physics fixes, source deletion based on apparent unused code, global dependency lists or Premake removal.

## Expected Changes

Link Application plus actual Render/Assets/Animation/Audio/UI roots and directly exposed SDK/package requirements; preserve all five current source files and startup properties.
Produce one understandable diff and `../reviews/BUILD_PHASE_36_REVIEW.md` with
...
Build Debug/Release, inspect actual flags/warnings/link closure, and launch from fresh staged CWD with restricted PATH. Exercise startup/panels, representative model/animation rendering, input and audio bank flow, then shutdown; compare reference and classify existing failures.
...
Verify expected executable name, window/UI properties, resource identities and user-visible startup behavior match Premake.
...
APPROVE BUILD PHASE 36 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-36-approved`
...
Verify `build-phase-35-approved` and its recorded peeled commit. Restore only the exact owned
...
Missing target/source/resource closure, first-party/link warnings, package/runtime mismatch or editor behavior changes.
