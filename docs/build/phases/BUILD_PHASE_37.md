# BUILD PHASE 37: Migrate RigidBodySimulation

## Phase Identity

- Track: Build System Migration; phase 37; status: NOT STARTED.
- Kind: **APPLICATION MIGRATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-36-approved`.
- Technical prerequisites: 36. These do not permit skipping the serial predecessor.

## Goal

Add the simulation executable with explicit Scene/Physics/render/audio integration.

## Why This Phase Exists

This app is the strongest real ECS/runtime-physics and renderer integration check and also initializes FMOD.

## Preconditions

Phase 36 is fully approved/published; HEAD equals build-phase-36-approved peeled commit; explicit START BUILD PHASE 37.
...
RigidBodySimulation/CMakeLists.txt; runtime/resource metadata; exact build-prerequisite includes only.
...
No physics solver tuning, rendering defect repair, changed timestep/lifecycle or new simulation features.

## Expected Changes

Link Application, Scene, Physics, Render/Geometry/Assets, UI and Audio as actually used; preserve source count, shader/icon/PBR/skybox/bank/CWD contracts.
Produce one understandable diff and `../reviews/BUILD_PHASE_37_REVIEW.md` with
...
Build both configs, inspect language/CRT/warnings and imports; run fresh staged startup and start/pause/stop/restart, selection/picking and resource/audio smoke. Compare fixed reference simulation inputs/outcomes and run relevant existing lifecycle tests.
...
Keep known lattice/contact/rendering defects classified against the source reference; require unchanged lifecycle/step outcomes.
...
APPROVE BUILD PHASE 37 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-37-approved`
...
Verify `build-phase-36-approved` and its recorded peeled commit. Restore only the exact owned
...
Physics/render outcome drift, missing resource/runtime closure or a requested fix exceeds build-only application migration.
