# BUILD PHASE 38: Migrate Breakout

## Phase Identity

- Track: Build System Migration; phase 38; status: NOT STARTED.
- Kind: **APPLICATION MIGRATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-37-approved`.
- Technical prerequisites: 37. These do not permit skipping the serial predecessor.

## Goal

Add Breakout with explicit 2D/sprite/input/audio and local-resource closure.

## Why This Phase Exists

Breakout is the Premake startproject and exercises a different renderer path and relative level/image layout.

## Preconditions

Phase 37 is fully approved/published; HEAD equals build-phase-37-approved peeled commit; explicit START BUILD PHASE 38.
...
Breakout/CMakeLists.txt; local level/image plus shared runtime/resource metadata; exact build-prerequisite includes only.
...
No gameplay, renderer2D, resource cache or input behavior fixes; no automatic physics-kernel dependency unless real source closure proves it.

## Expected Changes

Preserve four source files, EntryPoint/window settings, levels/images and explicit Audio/FMOD usage; link only actual module roots.
Produce one understandable diff and `../reviews/BUILD_PHASE_38_REVIEW.md` with
...
Build/launch fresh Debug/Release staging with restricted PATH; verify all four level files, background/paddle/ball/block textures, input transitions and audio flow. Inspect flags/warnings/imports and compare level startup/gameplay smoke to reference.
...
Retain source-CWD resource behavior and intended executable/startup semantics; document any already-known missing-resource case rather than silently editing assets.
...
APPROVE BUILD PHASE 38 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-38-approved`
...
Verify `build-phase-37-approved` and its recorded peeled commit. Restore only the exact owned
...
Relative resource handling differs, gameplay drift, undeclared module/runtime or reference bug needs implementation work.
