# BUILD PHASE 39: Migrate RayTracing

## Phase Identity

- Track: Build System Migration; phase 39; status: NOT STARTED.
- Kind: **APPLICATION MIGRATION**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-38-approved`.
- Technical prerequisites: 08, 38. These do not permit skipping the serial predecessor.

## Goal

Add RayTracing with the real software-render/TBB and platform/UI closure.

## Why This Phase Exists

Current links offer FMOD and many libraries despite no direct audio initialization; tracked historical objects must not mask a fresh build.

## Preconditions

Phase 38 is fully approved/published; HEAD equals build-phase-38-approved peeled commit; explicit START BUILD PHASE 39.
...
RayTracing/CMakeLists.txt; target runtime/resource metadata; narrow include spelling/prerequisite repairs for approved packages if needed.
...
No ray-tracing algorithm/performance change or reuse/deletion of tracked RayTracing/x64/Debug artifacts. No broad app template dependencies.

## Expected Changes

Preserve two current TUs and ray-tracing behavior; use explicit Application/Render/Assets/TBB/UI closure and exclude Audio/FMOD unless actual symbols prove necessity.
Produce one understandable diff and `../reviews/BUILD_PHASE_39_REVIEW.md` with
...
Build Debug/Release from empty outputs without legacy objects/bin; inspect TBB DLL/config closure and no gratuitous FMOD. Launch from staged CWD, exercise image generation and compare reference output/scene behavior with same inputs; inspect warnings/CRT/C++20.
...
Compare ray-tracing image/flow and UI startup while documenting needed TBB runtime packaging correction.
...
APPROVE BUILD PHASE 39 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-39-approved`
...
Verify `build-phase-38-approved` and its recorded peeled commit. Restore only the exact owned
...
Stale object used, unexplained FMOD dependency, TBB/version runtime mismatch or rendered output differs.
