# BUILD PHASE 42: Demonstrate full Premake and CMake equivalence

## Phase Identity

- Track: Build System Migration; phase 42; status: NOT STARTED.
- Kind: **FULL EQUIVALENCE GATE**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-41-approved`.
- Technical prerequisites: 41. These do not permit skipping the serial predecessor.

## Goal

Approve complete build/runtime/test equivalence before switching the authoritative build.

## Why This Phase Exists

Incremental target success is insufficient to retire the existing reference or claim all closures work from a fresh environment.

## Preconditions

Phase 41 is fully approved/published; HEAD equals build-phase-41-approved peeled commit; explicit START BUILD PHASE 42.
...
scripts/build_validation/ equivalence automation and docs/build/evidence/phase-42 reports/manifests; documentation corrections only. Production fixes need another phase.
...
No CMake authority switch or Premake removal. No baseline defect repair or blanket waiver. Compare current Premake and immutable Phase 00 reference; explain reviewed boundary changes.

## Expected Changes

Produce full target/config comparison matrix, source ownership/dependency DAG report, runtime/resource manifests and reproducible fresh-checkout procedure.
Produce one understandable diff and `../reviews/BUILD_PHASE_42_REVIEW.md` with
...
From fresh isolated source/build/package/staging roots using approved tool/SDK inputs, build all eight logical targets through both paths in Debug/Release. Verify every compiler C++20/CRT/option and warning record, static/module source ownership, imports/DLL/resources/CWD. Run full PhysicsTests and required selectors; benchmark normal/profiling/steady-state; launch all four apps through direct commands and driver. Test missing dependency/resource failure and safe clean. Require no stale bin/global PATH/previous build reliance.
...
All P4 dimensions must have explicit equivalence evidence. Baseline-known functional failures may remain only with exact unchanged signatures; language/CRT/first-party/linker warnings remain hard gates.
...
APPROVE BUILD PHASE 42 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-42-approved`
...
Verify `build-phase-41-approved` and its recorded peeled commit. Restore only the exact owned
...
Any unexplained difference, missing evidence, new warning/failure, cycle, duplicate/unowned TU, machine-state dependency or need for production fixes: propose a small additional phase before this gate.
