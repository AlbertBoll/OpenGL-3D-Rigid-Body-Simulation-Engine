# BUILD PHASE 43: Make CMake and Conan the authoritative build

## Phase Identity

- Track: Build System Migration; phase 43; status: NOT STARTED.
- Kind: **AUTHORITATIVE SWITCH**.
- Branch: build/cmake-conan-migration; source: render-refactor-phase-00-approved.
- Immediate predecessor / rejection checkpoint: `build-phase-42-approved`.
- Technical prerequisites: 42. These do not permit skipping the serial predecessor.

## Goal

Change documented/default development entrypoints to the fully validated CMake/Conan path.

## Why This Phase Exists

Authority should switch only after equivalence, while Premake remains a working fallback until a separate retirement review.

## Preconditions

Phase 42 is fully approved/published; HEAD equals build-phase-42-approved peeled commit; explicit START BUILD PHASE 43.
...
README/build onboarding docs; existing cli.py/tools entrypoint routing only if needed; build metadata/default CI configuration if one exists at execution; no engine source.
...
Do not delete Premake scripts/generator/vendor reference inputs or force users through an undocumented fallback. No default target/config changes beyond the approved interface.

## Expected Changes

Make scripts/gengine.py and direct CMake/Conan instructions canonical; label explicit Premake fallback and document tool/SDK/profile/runtime requirements and migration status.
Produce one understandable diff and `../reviews/BUILD_PHASE_43_REVIEW.md` with
...
Follow new-user setup/build/test/run instructions in a fresh output root for both configs; verify canonical commands and explicit Premake fallback still work. Check default/CI routing, exit behavior and docs against actual artifacts; confirm Phase 42 equivalence snapshot input compatibility.
...
Reference remains reproducible and available; this phase changes authority/entrypoints, not program behavior. State explicitly how to invoke fallback.
...
APPROVE BUILD PHASE 43 uses cheap snapshot/hash/Git checks only. Only explicit
...
POST_APPROVAL_HEAD != PRE_APPROVAL_HEAD, and annotate `build-phase-43-approved`
...
Verify `build-phase-42-approved` and its recorded peeled commit. Restore only the exact owned
...
Canonical instructions cannot reproduce the build, fallback is broken, or routing change exposes an unvalidated behavior difference.
