"""Validate typed asset identity, publication and CPU/GPU version retirement."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    try:
        ...
        if not invoke('compile-cpu', cpu_command, 120):
            return 1
        env['SDL_VIDEODRIVER'] = 'phase25-cpu-must-not-initialize-video'
        if not invoke('cpu', [cpu], marker='[PASS] asset-registry-CPU'):
            return 1
        for suffix, diagnostic in (('ACCESS', 'error C2664'), ('ASSIGN', 'error C2440')):
            if not invoke('reject-wrong-type-' + suffix.lower(),
                          [*common, '/DGENGINE_WRONG_HANDLE_' + suffix, '/c', source,
        ...
        gl = out / 'asset-registry-gl.exe'
        gl_command = [*common, '/DGENGINE_REGISTRY_GL', '/DSDL_MAIN_HANDLED', '/DGENGINE_PLATFORM_WINDOWS',
        ...
        report['inputs'] = {str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in
                            (cpu, gl, ROOT / 'bin' / config / 'GEngine/GEngine.lib', sdl)}
