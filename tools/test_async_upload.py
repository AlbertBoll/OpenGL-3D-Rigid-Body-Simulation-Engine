"""Validate Phase 49 async state, bounded storage and scheduler-only publication."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out = (args.output or ROOT / 'logs/rendering/phase49/final' / config).resolve()
    ...
    try:
        ...
        probe = ROOT / 'tools/async_upload_probe.cpp'
        native_free = [p for p in includes if not any(part in p.parts for part in ('sdl2', 'glad', 'assimp', 'tbb'))]
        boundary = [*flags, '/c', '/showIncludes', '/DUPLOAD_SCHEMA_ONLY', *['/I' + str(p) for p in native_free],
        ...
        if any(token in dependencies for token in ('/glad/', '/sdl2/', '/opengl/', '/imgui/', '/assimp/', '/gepch.h', 'backend.h')):
            ...
            return 1
        if not invoke('private-drain-negative', [*boundary, '/DUPLOAD_NEGATIVE_DRAIN'], expected='nonzero', marker='DrainUpdateFrameResources'):
            return 1
        changed = ['GEngine/include/GEngine/Assets/AsyncUploadQueue.h', 'GEngine/src/Assets/AsyncUploadQueue.cpp',
        ...
        for path in changed:
            ...
            if re.search(r'\b(throw|try|catch|exception_ptr|enable_if)\b', source):
                ...
                return 1
        report['architecture'] = {'native_free_consumer': 'PASS', 'private_drain': 'PASS', 'no_new_exceptions': 'PASS'}
        executable = out / 'async-upload-probe.exe'
        ...
        if not invoke('async-upload', [executable], cwd=out, marker='[PASS] async-upload'):
            return 1
        if not invoke('worker-gl-negative', [executable, '--worker-gl'], cwd=out, expected='nonzero',
                      marker='async negative worker upload requires the owning context thread'):
            return 1
        if not invoke('context-switch-negative', [executable, '--switch-context'], cwd=out, expected='nonzero',
                      marker='[negative] upload returned on another registered context'):
            return 1
        passed = True
        return 0
    finally:
