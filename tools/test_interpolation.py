"""Build affected consumers and validate simulation-to-render interpolation on Windows."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out = (args.output or ROOT / "logs/rendering/phase17" / config).resolve()
    ...
    try:
        ...
        executable = out / "interpolation-probe.exe"
        command = [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++20", "/EHsc", "/W3",
                   ...
                   *["/external:I" + str(p) for p in (ROOT / "external", ROOT / "GEngine/include/external", vc / "include")],
                   ROOT / "tools/interpolation_probe.cpp",
                   "/Fo" + str(out) + os.sep,
        ...
        for mode in ("--fixed-scheduling", "--runtime-transform"):
            outcomes.append(invoke(mode[2:], [physics, mode], timeout=180))
        passed = all(outcomes)
