"""Validate Phase 35 material authoring, resource preparation, packing and retained lifetimes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out = (args.output or ROOT / 'logs/rendering/phase35/final' / config).resolve()
    ...
    try:
        ...
        common = [vc / 'bin/Hostx64/x64/cl.exe', '/nologo', '/std:c++23preview', '/EHsc', '/W4',
                  ...
                  '/external:W0', '/external:I' + str(ROOT / 'GEngine/include/external')]
        cpu = out / 'material-instance-cpu.exe'
        cpu_link = ['/link', '/SUBSYSTEM:CONSOLE', *['/LIBPATH:' + str(p) for p in libraries],
                    ROOT / 'bin' / config / 'GEngine/GEngine.lib']
        if not invoke('consumer-boundary', [*common, '/WX', '/showIncludes', ROOT / 'tools/material_instance_probe.cpp',
        ...
        if any(token in dependencies for token in ('/glad/', '/sdl2/', '/opengl/', '/imgui/', '/assimp/', '/gepch.h', 'backend.h')):
            ...
            return 1
        production = ['GEngine/include/GEngine/Material/' + p for p in ('MaterialInstance.h', 'MaterialBinding.h', 'MaterialTemplate.h')]
        production += ['GEngine/src/Material/' + p for p in ('MaterialInstance.cpp', 'MaterialBinding.cpp', 'MaterialTemplate.cpp')]
        for relative in production:
            content = (ROOT / relative).read_text(encoding='utf-8')
            content = re.sub(r'//[^\n]*|/\*.*?\*/', '', content, flags=re.S)
            if re.search(r'\b(throw|try|catch|enable_if)\b', content):
        ...
        env['SDL_VIDEODRIVER'] = 'phase35-cpu-must-not-initialize-video'
        if not invoke('authoring-cpu', [cpu], marker='[PASS] material-instance'):
            return 1
        env.pop('SDL_VIDEODRIVER', None)
        ...
        gl_common = [*common, '/DSDL_MAIN_HANDLED', '/DGENGINE_PLATFORM_WINDOWS', '/DMATERIAL_INSTANCE_GL_PROBE',
        ...
        gpu = out / 'material-instance-gl.exe'
        if not invoke('compile-resource-probe', [*gl_common, ROOT / 'tools/material_instance_probe.cpp',
                      '/Fo' + str(out / 'gpu.obj'), '/Fe' + str(gpu), *link]):
            return 1
        env['Path'] = str(ROOT / 'bin' / config / 'GEngineEditor') + os.pathsep + env['Path']
        if not invoke('resources-packing-lifetime', [gpu], marker='[PASS] material-instance'):
            return 1
        if not invoke('template-regression', [sys.executable, ROOT / 'tools/test_material_pipeline.py',
                      '--configuration', config, '--output', out / 'declarations'], 240, marker='[PASS] results:'):
            return 1
        report['inputs'] = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest() for p in
                           [*(ROOT / p for p in production), ROOT / 'tools/material_instance_probe.cpp',
                            ROOT / 'tools/test_material_instance.py', gpu, cpu, ROOT / 'bin' / config / 'GEngine/GEngine.lib']}
        passed = True
        return 0
