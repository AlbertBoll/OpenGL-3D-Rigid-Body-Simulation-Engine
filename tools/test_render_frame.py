...
def main():
    ...
    try:
        ...
        consumer = ROOT / "tools/render_frame_probe.cpp"
        ...
        if not invoke("frame-consumer-boundary", [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++23preview",
                      ...
                      "/DFRAME_SCHEMA_ONLY", "/D_SILENCE_CXX23_ALIGNED_STORAGE_DEPRECATION_WARNING",
        ...
        if any(token in dependencies for token in ("/glad/", "/sdl2/", "/opengl/", "/imgui/", "/assimp/", "/gepch.h", "backend.h")):
            report["reason"] = "Concrete backend declaration leaked into the frame consumer"
            return 1
        report["consumer_boundary"] = "PASS"
        for name, statement in {
            "draw-mutation": "frame.Draws()[0].sortKey = 3;",
            "camera-mutation": "frame.Cameras()[0].viewportWidth = 1;",
            "debug-mutation": "frame.DebugLines()[0].color.a = 0;",
            "resource-mutation": "frame.Resources()[0].Mesh() = {};",
            "copy-frame": "auto copy = frame;",
            "temporary-view": "auto view = std::move(frame).Draws();",
        }.items():
            source = out / (name + ".cpp")
            source.write_text('#include "Renderer/RenderFrame.h"\nvoid Invalid(GEngine::RenderFrame& frame) { '
                              + statement + ' }\n')
        ...
        executable = out / "render-frame-probe.exe"
        command = [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++23preview", "/EHsc", "/W3",
                   ...
                   *["/external:I" + str(p) for p in (ROOT / "external", ROOT / "GEngine/include/external", vc / "include")],
                   ROOT / "tools/render_frame_probe.cpp",
                   "/Fo" + str(out) + os.sep,
        ...
        passed = invoke("render-frame", [executable], cwd=out,
                        marker="[PASS] render-frame ")
