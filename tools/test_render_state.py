"""Build affected consumers and validate presentation bounds and revision sources."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    try:
        ...
        consumer.write_text('#include <Scene/_Entity.h>\n'
                            ...
                            'std::expected<GEngine::SceneRenderState, GEngine::TransformError> '
                            'Snapshot(GEngine::_Scene& scene, const GEngine::RenderStateResources& r) { return scene.UpdateRenderState(r); }\n')
        ...
        command = [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++23preview", "/EHsc", "/W3",
                   ...
                   *["/external:I" + str(p) for p in (ROOT / "external", ROOT / "GEngine/include/external", vc / "include")],
                   ROOT / "tools/render_state_probe.cpp",
                   "/Fo" + str(out) + os.sep,
