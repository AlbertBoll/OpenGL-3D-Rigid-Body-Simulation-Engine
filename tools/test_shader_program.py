"""Build affected consumers and validate transactional shader ownership on real GL."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out = (args.output or ROOT / "logs/rendering/phase24" / config).resolve()
    ...
    def invoke(name, command, timeout=60, expected_exit=0, marker=None, cwd=ROOT):
        command = [str(value) for value in command]
        log = out / (name + ".log")
        step = {"name": name, "command": command, "cwd": str(cwd), "log": str(log), "expected_exit": expected_exit, "required_output": marker}
    ...
    try:
        ...
        if not invoke("generate", [ROOT / "vendor/bin/premake/premake5.exe", "vs2022"], 120):
            return 1
        build = [msbuild, ROOT / "GEngine.sln",
        ...
        executable = out / "shader-program-probe.exe"
        command = [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++20", "/EHsc", "/W3",
                   ...
                   *["/external:I" + str(p) for p in (ROOT / "external", ROOT / "GEngine/include/external", vc / "include")],
                   ROOT / "tools/shader_program_probe.cpp",
                   "/Fo" + str(out) + os.sep,
        ...
        if not invoke("compile-probe", command, 120):
            return 1
        manager_executable = out / "shutdown-probe.exe"
        manager_command = [str(value).replace("shader_program_probe.cpp", "shutdown_probe.cpp")
                           .replace(str(executable), str(manager_executable)) for value in command]
        if not invoke("compile-manager-probe", manager_command, 120):
            return 1
        sdl = ROOT / "bin" / config / "GEngineEditor/SDL2.dll"
        report["inputs"] = {str(p): hashlib.sha256(p.read_bytes()).hexdigest() for p in
                            (executable, manager_executable, ROOT / "bin" / config / "GEngine/GEngine.lib", sdl)}
        env["Path"] = str(sdl.parent) + os.pathsep + env["Path"]
        runtime = out / "runtime"
        runtime.mkdir(exist_ok=True)
        outcomes = [invoke("probe", [executable], marker="[PASS] shader-program-RAII cycles=2", cwd=runtime)]
        if config == "Debug":
            outcomes.append(invoke("reject-delete", [executable, "--reject-delete"], expected_exit=86,
                                   marker="[GLThread] assertion failed:", cwd=runtime))
        manager_runtime = out / "manager-runtime"
        manager_runtime.mkdir(exist_ok=True)
        env["GENGINE_ASSET_ROOT"] = str(ROOT / "bin" / config / "assets")
        env["GENGINE_SHADOW_RESOLUTION"] = "32"
        outcomes.append(invoke("manager-failures", [manager_executable, "--manager-failures"],
                               timeout=180, marker="[PASS] shutdown --manager-failures", cwd=manager_runtime))
        passed = all(outcomes)
