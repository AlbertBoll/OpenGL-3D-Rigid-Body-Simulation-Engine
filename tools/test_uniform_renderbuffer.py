"""Build affected consumers and validate uniform/renderbuffer ownership on real GL."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    out = (args.output or ROOT / "logs/rendering/phase23" / config).resolve()
    ...
    def invoke(name, command, timeout=60, expected_exit=0, marker=None):
        command = [str(value) for value in command]
        log = out / (name + ".log")
        step = {"name": name, "command": command, "cwd": str(ROOT), "log": str(log), "expected_exit": expected_exit, "required_output": marker}
    ...
    try:
        ...
        executable = out / "uniform-renderbuffer-probe.exe"
        command = [vc / "bin/Hostx64/x64/cl.exe", "/nologo", "/std:c++20", "/EHsc", "/W3",
                   ...
                   *["/external:I" + str(p) for p in (ROOT / "external", ROOT / "GEngine/include/external", vc / "include")],
                   ROOT / "tools/uniform_renderbuffer_probe.cpp",
                   "/Fo" + str(out) + os.sep,
        ...
        outcomes = [invoke("probe", [executable], marker="[PASS] uniform-renderbuffer-RAII cycles=2")]
        if config == "Debug":
            for mode in ("--reject-ubo-delete", "--reject-rbo-delete"):
                outcomes.append(invoke(mode[2:], [executable, mode], expected_exit=86, marker="[GLThread] assertion failed:"))
        passed = all(outcomes)
